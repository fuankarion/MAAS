...
import core.models.maas as maas

from torch.optim import lr_scheduler
from torch_geometric.data import DataLoader

from core.util import set_up_log_and_ws_out
from core.maas_datasets import TimeAssignationFullySupervised, TimeAssignationFullySupervisedNoise
from core.optimization import optimize_assignation_gcn
...
if __name__ == '__main__':
    ...
    io_config = exp_conf.MAAS_inputs_160
    opt_config = exp_conf.MAAAS_optimization_params

    # io config
    model_name = '160_MAAS_'+str(time_length)+'len_'+str(stride)+'stride_'+str(speakers)+'speakers_'+str(hidden_size)+'filters_'+str(clusters)+'k'
    ...
    dataset_train = TimeAssignationFullySupervisedNoise(io_config['features_train_full'], time_length, stride,
                                       noise=0.2, candidate_speakers=speakers,
                                       limit_videos = None)
    dataset_val = TimeAssignationFullySupervised(io_config['features_val_full'], time_length, stride,
    ...
    optimize_assignation_gcn(model, (1, speakers, time_length), dl_train, dl_val, device,
